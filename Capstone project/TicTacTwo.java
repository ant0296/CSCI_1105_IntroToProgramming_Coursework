...
class TicTacTwo {
	...
	public static String[][] playPiece(String[][]gameBoard, String player, int size) {
		...
		do {
			...
			if (row >= size || column >= size) {
				playerCheck = true;
				System.out.println("One of those coordinates are a bit bigger than the current board size, try placing a piece somewhere else.");
			}
	 ...
	 * @param a int value, used to scan the array with 'b'.
	 * @param b int value, used to scan the array with 'a'.
	 * @param key A string value that I put in back when I was trying a different testing style and never removed.
